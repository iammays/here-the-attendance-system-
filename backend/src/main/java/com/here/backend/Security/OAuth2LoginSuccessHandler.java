...
import com.shouq.project.Security.jwt.JwtUtils;
...
public class OAuth2LoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        ...
        Cookie idCookie = new Cookie("id", String.valueOf(user.getUserID()));
        idCookie.setPath("/");
        ...
        response.getWriter().write(String.format("{\"email\": \"%s\", \"accessToken\": \"%s\", \"username\": \"%s\", \"id\": \"%d\"}", 
                                                  username, jwt, user.getUsername(), user.getUserID()));
